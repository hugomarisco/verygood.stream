...
describe("PreciseTimestamp", () => {
  ...
  test("should default to current time", () => {
    ...
    const expected = Buffer.from([0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x07, 0xd0]);
  ...
  test("should subtract a timestamp that doesn't overflow the microseconds", () => {
    const result = new PreciseTimestamp([1, 2000]).minus(new PreciseTimestamp([0, 1000]));
  ...
  test("should subtract a timestamp that overflows the microseconds", () => {
    const result = new PreciseTimestamp([1, 2000]).minus(new PreciseTimestamp([0, 3000]));

    expect(result.seconds).toBe(0);
    expect(result.microseconds).toBe(9000);
  });
});
